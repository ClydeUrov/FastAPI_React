...
from backend.models import CarDB, CarUpdate
...
async def create_car(request: Request, car: CarBase = Body(...)):
    ...
    create_car = await request.app.mongodb['cars1'].find_one({"_id": new_car.inserted_id})
...
async def show_car(id: str, request: Request):
    if (car := await request.app.mongodb["cars1"].find_one({"_id": id})) is not None:
        return CarDB(**car)
    raise HTTPException(status_code=404, detailt=f"Car with {id} not found")
